/*
 * @Author: delevin.ying 
 * @Date: 2023-04-27 16:53:37 
 * @Last Modified by: delevin.ying
 * @Last Modified time: 2023-04-27 17:03:48
 */
...
public class LoadBase
{
    ...
    protected LoadType type = LoadType.Null;
    ...
    internal void SubRefCount()
    {
        ...
        {
            LogMaster.E("资源未被加载，引用不可能减少\n" + FilePath);
            return;
        }
        if (_loadState == LoadState.Loading)
        {
            LogMaster.L("资源加载中，等加载完成后再进入卸载逻辑\n" + FilePath);
            return;
        }
        if (_refCount <= 0)
        {
    ...
    internal void LoadAssetBundle(string bundlePackageName)
    {
        ...
        if (_loadState == LoadState.Loading)
        {
            LogMaster.E("同步加载了正在异步加载的资源, 打断异步加载资源会导致所有异步加载的资源都立刻同步加载出来。资源名: " + FilePath +
    ...
    internal void ForceLoadFinish(string bundlePackageName)
    {
        ...
        if (_assetBundleCreateRequest != null)
        {
            LogMaster.E("触发强制加载, 打断异步加载资源会导致所有异步加载的资源都立刻同步加载出来。资源名: " + FilePath +
                                   "\nAssetBundle包名: " + AssetBundleName);
            AssetBundle = _assetBundleCreateRequest.assetBundle;
            return;
        }
