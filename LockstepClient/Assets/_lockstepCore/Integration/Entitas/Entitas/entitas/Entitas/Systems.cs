...
namespace Entitas
{
    ...
    public class Systems : IInitializeSystem, ISystem, IExecuteSystem, ICleanupSystem, ITearDownSystem
    {
        ...
        {
            if (system is IInitializeSystem item)
            {
                _initializeSystems.Add(item);
            }
            if (system is IExecuteSystem item2)
            {
                _executeSystems.Add(item2);
            }
            if (system is ICleanupSystem item3)
            {
                _cleanupSystems.Add(item3);
            }
            if (system is ITearDownSystem item4)
            {
                _tearDownSystems.Add(item4);
            }
            return this;
        }
        ...
        public void ActivateReactiveSystems()
        {
            for (int i = 0; i < _executeSystems.Count; i++)
            {
                IExecuteSystem executeSystem = _executeSystems[i];
                if (executeSystem is IReactiveSystem reactiveSystem)
                {
                    reactiveSystem.Activate();
                }
                if (executeSystem is Systems systems)
                {
