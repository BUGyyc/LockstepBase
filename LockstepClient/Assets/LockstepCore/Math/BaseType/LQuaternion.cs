...
namespace Lockstep
{
    public struct LQuaternion
    {
        ...
        public static LFloat Angle(LQuaternion a, LQuaternion b)
        {
            ...
            //正轴的余弦值
            LFloat val = LMath.Min(LMath.Abs(single), LFloat.one);
            //得到弧度
            LFloat flAcos = LMath.Acos(val);

            LFloat flBase = 2 * (180 / LMath.PI);
            //弧度转角度
            var value = flAcos * flBase;

            //UnityEngine.Debug.LogFormat("[RotReach]  value {0}  val {1} flAcos {2} flBase {3}", value, val, flAcos, flBase);
            return value;
        }
