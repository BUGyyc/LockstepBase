...
#    1  | required/optional | repeated  | required_struct/optional_struct/struct   |
#       | ------------------| ---------:| ----------------------------------------:|
#    2  | 属性类型          |最大个数   | 结构元素个数                             |
#    3  | 属性名            |           | 结构类型名                               |
#    4  | 注释说明          |           | 在上层结构中的属性名                     |
...
class SheetInterpreter:
    ...
    def __init__(self, xls_file_path, sheet_name):
        ...
        self._sheet_name = sheet_name + "Cfg"
    ...
    def Interpreter(self) :
        ...
        # 将PB转换成py格式
        try :
            command = "protoc --python_out=. " + self._pb_file_name + " --experimental_allow_proto3_optional"
            os.system(command)
        except BaseException as e :
            print("protoc failed!")
            raise
    ...
    def _FieldDefine(self, repeated_num) :
        ...
        if field_rule == "required" or field_rule == "optional" :
            ...
            field_comment = str(self._sheet.cell_value(FIELD_COMMENT_ROW, self._col))
        ...
        elif field_rule == "required_struct" or field_rule == "optional_struct" or field_rule == "struct" :
            #字段数和重复次数
            f_and_r_num = str(self._sheet.cell_value(FIELD_TYPE_ROW, self._col)).split("-", 1)
            
            if len(f_and_r_num) != 2:
                raise ValueError("格式错误，请参考格式f-r，f为字段数，r为最大重复次数，用-分隔")
            
            #repeated处理，减少一列配置
            repeated_num = int(f_and_r_num[1])

            
            field_num = int(f_and_r_num[0])
    ...
    def _LayoutArray(self) :
        """输出数组定义"""
        self._output.append("message " + self._sheet_name + "Sheet {\n")
        self._output.append("    map<uint32, " + self._sheet_name + "> item_dic = 1;\n}\n")
...
class DataParser:
    ...
    def __init__(self, xls_file_path, sheet_name):
        ...
        try:
            self._module_name = OUTPUT_FULE_PATH_BASE + self._sheet_name.lower().replace("cfg","") + "_pb2"
    ...
    def Parse(self) :
        ...
        item_dic = getattr(self._module, self._sheet_name+'Sheet')()
        ...
        for self._row in range(4, self._row_count) :
            # 如果 id 是 空 直接跳过改行
            cell_value = self._sheet.cell_value(self._row, id_col);
            info_id = str(cell_value).strip()
            if info_id == "" :
                LOG_WARN("%d is None", self._row)
                continue
            item = item_dic.item_dic[int(cell_value)];
            self._ParseLine(item)

        LOG_INFO("parse result:\n%s", item_dic)

        self._WriteReadableData2File(str(item_dic).encode('utf-8'))

        data = item_dic.SerializeToString(deterministic=True)
    ...
    def _ParseField(self, max_repeated_num, repeated_num, item) :
        ...
        if field_rule == "required" or field_rule == "optional" :
            ...
            #repeated处理，减少一列配置
            max_repeated_num = int(float(f_and_r_num[1]))
            read = self._sheet.cell_value(self._row, self._col)
            repeated_num = max_repeated_num if read == "" else int(self._sheet.cell_value(self._row, self._col))
        ...
        try:
            if field_type == "int32" or field_type == "int64"\
                ...
                if len(field_value) <= 0 :
                    ...
                    #justinjin modify for KLP
                    #here regard time as GMT time instead of local time
